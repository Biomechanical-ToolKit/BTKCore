...
if ~isequal(size(params.A), size(params.B))
    message = comparisonMessage(params.Message, ...
        'Inputs are not the same size.', ...
        params.A, params.B);
    error('assertElementsAlmostEqual:sizeMismatch', ...
        '%s', message);
end
...
if ~(isfloat(params.A) && isfloat(params.B))
    message = comparisonMessage(params.Message, ...
        ...
        params.A, params.B);
    error('assertElementsAlmostEqual:notFloat', ...
        '%s', message);
end

if ~compareFloats(params.A, params.B, 'elementwise', ...
    ...
    message = comparisonMessage(params.Message, tolerance_message, ...
        params.A, params.B);
    
    error('assertElementsAlmostEqual:tolExceeded', ...
        '%s', message);
end
