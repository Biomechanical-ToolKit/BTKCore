...
CXXTEST_SUITE(EventTest)
{
  ...
  CXXTEST_TEST(FindChildren)
  {
    ...
    TS_ASSERT_EQUALS(events.findChildren<btk::Event*>({},{{"time",0.0}},false).size(),2);
    TS_ASSERT_EQUALS(events.findChildren<btk::Event*>({},{{"context","Right"}},false).size(),2);
    TS_ASSERT_EQUALS(events.findChildren<btk::Event*>({},{{"context","Left"}},false).size(),2);
    TS_ASSERT_EQUALS(events.findChildren<btk::Event*>({},{{"subject","JDoe"}},false).size(),3);
    ...
    TS_ASSERT_EQUALS(events.findChildren<btk::Event*>(std::regex("Foo|Bar")).size(),2);
    TS_ASSERT_EQUALS(events.findChildren<btk::Event*>(std::regex("Foo|Bar|Toto")).size(),4);
    TS_ASSERT_EQUALS(events.findChildren<btk::Event*>(std::regex("Foo|Bar|Toto"),{{"context","Right"}}).size(),2);
  };
};
